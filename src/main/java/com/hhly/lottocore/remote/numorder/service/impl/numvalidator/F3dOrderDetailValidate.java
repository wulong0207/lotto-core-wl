...
/**
 * @desc 福彩3D订单明细校验(仅提供明细内容验证，其它验证统一在主流程处理)
 * @author huangb
 * @date 2017年6月14日
 * @company 益彩网络
 * @version v1.0
 */
@Component("f3dOrderDetailValidate")
public class F3dOrderDetailValidate extends OrderDetailValidate {

	/**
	 * 日志对象
	 */
	private static Logger logger = LoggerFactory.getLogger(F3dOrderDetailValidate.class);
	
	@Override
	protected ResultBO<?> verifyContentType(OrderDetailVO orderDetail) throws ResultJsonException {
		logger.debug("F3dOrderDetailValidate verifyContentType begin!");
		...
		case D_DIRECT:
			...
			switch (type) {
			case SINGLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_DIRECT_SINGLE, planContent), "40402", planContent);
				break;
			case MULTIPLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_DIRECT_MULTIPLE, planContent), "40402", planContent);
				break;
			case SUM:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_DIRECT_SUM, planContent), "40402", planContent);
				break;
			default:
		...
		case D_G3:
			...
			switch (type) {
			case SINGLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G3_SINGLE, planContent), "40402", planContent);
				break;
			case MULTIPLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G3_MULTIPLE, planContent), "40402", planContent);
				break;
			case SUM:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G3_SUM, planContent), "40402", planContent);
				break;
			default:
				throw new ResultJsonException(ResultBO.err("40403"));
			}
		...
		case D_G6:
			...
			switch (type) {
			case SINGLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G6_SINGLE, planContent), "40402", planContent);
				break;
			case MULTIPLE:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G6_MULTIPLE, planContent), "40402", planContent);
				break;
			case SUM:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G6_SUM, planContent), "40402", planContent);
				break;
			case DANTUO:
				Assert.isTrue(Pattern.matches(NUMConstants.F3D_REGEX_G6_DANTUO, planContent), "40402", planContent);
				break;
			default:
				throw new ResultJsonException(ResultBO.err("40403"));
			}
	...
	protected BetContentVO genBetContent(OrderDetailVO orderDetail) throws ResultJsonException {
		logger.debug("F3dOrderDetailValidate genBetContent begin!");
		// 投注内容
		String planContent = orderDetail.getPlanContent();
		// 子玩法类型
		LotteryChild lotChild = LotteryChild.valueOf(orderDetail.getLotteryChildCode());
	...
	protected int genBetNum(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3dOrderDetailValidate genBetNum begin!");
		...
		case D_DIRECT:
			...
			case SUM:
				// 注数 = 各和值对应注数之和
				return getSumTotalNum(betContent.getArea1(), NUMConstants.F3D_DIRECT_SUM_BETNUM);
			default:
				throw new ResultJsonException(ResultBO.err("40403"));
			}
		case D_G3:
			...
			case SUM:
				// 注数 = 各和值对应注数之和
				return getSumTotalNum(betContent.getArea1(), NUMConstants.F3D_G3_SUM_BETNUM);
			default:
				throw new ResultJsonException(ResultBO.err("40403"));
			}
		case D_G6:
			...
			case MULTIPLE:
				int area1Len = betContent.getArea1().length;// 选号个数
				// 注数 = 选号个数中选3个的组合数
				return (int) NumberUtil.getCombinationCount(area1Len, NUMConstants.F3D_CHOOSE_3);
			case SUM:
				// 注数 = 各和值对应注数之和
				return getSumTotalNum(betContent.getArea1(), NUMConstants.F3D_G6_SUM_BETNUM);
			case DANTUO:
				int danLen = betContent.getArea1().length;// 胆个数
				int tuoLen = betContent.getArea2().length;// 拖个数
				// 注数 = 拖选号个数中选（3-胆个数）个的组合数
				return (int) NumberUtil.getCombinationCount(tuoLen, (NUMConstants.F3D_CHOOSE_3 - danLen));
			default:
				throw new ResultJsonException(ResultBO.err("40403"));
			}
		default:
	...
	protected ResultBO<?> verifyBetContent(OrderDetailVO orderDetail, BetContentVO betContent)
		...
		logger.debug("F3dOrderDetailValidate verifyBetContent begin!");
	...
	private ResultBO<?> verifyDIRSingle(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyDIRSingle begin!");
		// 验证百、十、个区域基本信息:
		...
		logger.debug("F3D verifyDIRSingle end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验直选-复式投注内容
	...
	private ResultBO<?> verifyDIRMultiple(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyDIRMultiple begin!");
		// 验证百、十、个区域基本信息:
		String[] area1Arr = betContent.getArea1();// 百位号码数
		String[] area2Arr = betContent.getArea2();// 十位号码数
		String[] area3Arr = betContent.getArea3();// 个位号码数
		...
		Assert.isTrue(validLenRange((area1Arr.length + area2Arr.length + area3Arr.length), NUMConstants.F3D_CHOOSE_4, NUMConstants.F3D_CHOOSE_30), "40402", orderDetail.getPlanContent());
		...
		logger.debug("F3D verifyDIRMultiple end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验直选-和值投注内容
	...
	private ResultBO<?> verifyDIRSum(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyDIRSum begin!");
		// 验证和值基本信息:
		...
		// 2.选号的数字范围(0-27)
		for (String area1 : area1Arr) {
			Assert.isTrue(validNumRange(area1, NUMConstants.F3D_DIRECT_SUM_MIN_0, NUMConstants.F3D_DIRECT_SUM_MAX_27), "40563", orderDetail.getPlanContent(), area1, NUMConstants.F3D_DIRECT_SUM_MIN_0, NUMConstants.F3D_DIRECT_SUM_MAX_27);
		}

		// 3.选号重复性(不可重复)
		Assert.isTrue(!ArrayUtil.isRepeat(area1Arr), "40564", orderDetail.getPlanContent());

		logger.debug("F3D verifyDIRSum end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组三-单式投注内容
	...
	private ResultBO<?> verifyG3Single(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG3Single begin!");
		...
		logger.debug("F3D verifyG3Single end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组三-复式投注内容
	...
	private ResultBO<?> verifyG3Multiple(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG3Multiple begin!");
		...
		if (betContent.getArea1() != null && betContent.getArea2() != null) {
			...
			Assert.isTrue(validLenRange((area1Arr.length + area2Arr.length), NUMConstants.F3D_CHOOSE_3, NUMConstants.F3D_CHOOSE_20), "40402", orderDetail.getPlanContent());
		...
		logger.debug("F3D verifyG3Multiple end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组三-和值投注内容
	...
	private ResultBO<?> verifyG3Sum(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG3Sum begin!");
		// 验证和值基本信息:
		String[] area1Arr = betContent.getArea1();// 和值号码
		...
		// 2.选号的数字范围(1-26)
		for (String area1 : area1Arr) {
			Assert.isTrue(validNumRange(area1, NUMConstants.F3D_G3_SUM_MIN_1, NUMConstants.F3D_G3_SUM_MAX_26), "40572", orderDetail.getPlanContent(), area1, NUMConstants.F3D_G3_SUM_MIN_1, NUMConstants.F3D_G3_SUM_MAX_26);
		}

		// 3.选号重复性(不可重复)
		Assert.isTrue(!ArrayUtil.isRepeat(area1Arr), "40573", orderDetail.getPlanContent());

		logger.debug("F3D verifyG3Sum end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组六-单式投注内容
	...
	private ResultBO<?> verifyG6Single(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG6Single begin!");
		// 验证选号基本信息:
		String[] area1Arr = betContent.getArea1();// 选号
		...
		logger.debug("F3D verifyG6Single end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组六-复式投注内容
	...
	private ResultBO<?> verifyG6Multiple(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG6Multiple begin!");
		// 验证选号基本信息:
		String[] area1Arr = betContent.getArea1();// 选号
		...
		logger.debug("F3D verifyG6Multiple end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组六-和值投注内容
	...
	private ResultBO<?> verifyG6Sum(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG6Sum begin!");
		// 验证和值基本信息:
		String[] area1Arr = betContent.getArea1();// 和值号码
		...
		// 2.选号的数字范围(3-24)
		for (String area1 : area1Arr) {
			Assert.isTrue(validNumRange(area1, NUMConstants.F3D_G6_SUM_MIN_3, NUMConstants.F3D_G6_SUM_MAX_24), "40581", orderDetail.getPlanContent(), area1, NUMConstants.F3D_G6_SUM_MIN_3, NUMConstants.F3D_G6_SUM_MAX_24);
		}

		// 3.选号重复性(不可重复)
		Assert.isTrue(!ArrayUtil.isRepeat(area1Arr), "40582", orderDetail.getPlanContent());

		logger.debug("F3D verifyG6Sum end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 校验组六-胆拖投注内容
	...
	private ResultBO<?> verifyG6DT(OrderDetailVO orderDetail, BetContentVO betContent) throws ResultJsonException {
		logger.debug("F3D verifyG6DT begin!");
		// 验证胆、拖区域基本信息:
		...
		Assert.isTrue(validLenRange(area1Arr.length + area2Arr.length, NUMConstants.F3D_CHOOSE_3, NUMConstants.F3D_CHOOSE_10), "40585", orderDetail.getPlanContent(), NUMConstants.F3D_CHOOSE_3, NUMConstants.F3D_CHOOSE_10);
		...
		logger.debug("F3D verifyG6DT end!");
		return ResultBO.ok();
	}
		...
		try {
			...
			for (LimitNumberInfoBO limit : limitList) {
				for (LimitNumberDetailBO limitDtl : limit.getLimitNumberList()) {
					...
					if (ObjectUtil.isBlank(limitNumList) || limitNumList.size() != NUMConstants.F3D_CHOOSE_3) {
						logger.info("配置的限号内容：" + limitDtl.getLimitContent() + ",不符合规则，只能配置单式内容类型，忽略该配置项！！！！");
						continue;
					}
					// 1.3.分子玩法-内容类型分别验证限号(注：这里用if，不用switch，判断更直观)
					if (lotChild.getValue() == LotteryChild.D_DIRECT.getValue()) {
					...
					} else if (lotChild.getValue() == LotteryChild.D_G3.getValue()) {
					...
					} else if (lotChild.getValue() == LotteryChild.D_G6.getValue()) {
	...
	private ResultBO<?> verifyLimitDIRDefault(OrderDetailVO orderDetail, BetContentVO betContent,
		...
		logger.debug("F3D verifyLimitDIRDefault begin!");
		// 验证百、十、个区域限号信息:
		String[] area1Arr = betContent.getArea1();// 百位号码数
		String[] area2Arr = betContent.getArea2();// 十位号码数
		String[] area3Arr = betContent.getArea3();// 个位号码数
		...
		logger.debug("F3D verifyLimitDIRDefault end!");
		return ResultBO.ok();
	}
	...
	private ResultBO<?> verifyLimitG3Single(OrderDetailVO orderDetail, BetContentVO betContent,
			List<String> limitNumList) throws ResultJsonException {
		logger.debug("F3D verifyLimitG3Single begin!");
		// 20170701 add 单式两种格式（1,1,0 或 1|0）(第一种投注内容只有一个区域；第二种投注内容有两个区域)
		...
		logger.debug("F3D verifyLimitG3Single end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 验证组三(复式)限号
	...
	private ResultBO<?> verifyLimitG3Multiple(OrderDetailVO orderDetail, BetContentVO betContent,
			List<String> limitNumList) throws ResultJsonException {
		logger.debug("F3D verifyLimitG3Multiple begin!");
		// 20170701 add 复式两种格式（0,1,2,3 或 0,1,2,3|0,1,2,3）(第一种投注内容只有一个区域；第二种投注内容有两个区域)
		...
		logger.debug("F3D verifyLimitG3Multiple end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 验证组六(单式、复式)限号
	...
	private ResultBO<?> verifyLimitG6Default(OrderDetailVO orderDetail, BetContentVO betContent,
			List<String> limitNumList) throws ResultJsonException {
		logger.debug("F3D verifyLimitG6Default begin!");
		// 验证组六限号信息:
		...
		logger.debug("F3D verifyLimitG6Default end!");
		return ResultBO.ok();
	}
	...
	private ResultBO<?> verifyLimitSum(OrderDetailVO orderDetail, BetContentVO betContent, List<String> limitNumList)
			throws ResultJsonException {
		logger.debug("F3D verifyLimitSum begin!");
		// 验证和值基本信息:
		String[] area1Arr = betContent.getArea1();// 和值号码
		...
		logger.debug("F3D verifyLimitSum end!");
		return ResultBO.ok();
	}
	/**
	 * @desc 验证组六(胆拖)限号
	...
	private ResultBO<?> verifyLimitG6DT(OrderDetailVO orderDetail, BetContentVO betContent,
			List<String> limitNumList) throws ResultJsonException {
		logger.debug("F3D verifyLimitG6DT begin!");
		// 验证胆、拖限号:
		String[] area1Arr = betContent.getArea1();// 胆区号码
		String[] area2Arr = betContent.getArea2();// 拖区号码
		...
		logger.debug("F3D verifyLimitG6DT end!");
		return ResultBO.ok();
	}
}
