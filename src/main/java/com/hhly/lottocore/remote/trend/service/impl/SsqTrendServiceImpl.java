...
import com.hhly.skeleton.lotto.base.trend.num.bo.SsqTrendBO;
 ...
 * @version
 */
@Service("ssqTrendService")
public class SsqTrendServiceImpl extends NumTrendServiceImpl implements ISsqTrendService {
	...
	public ResultBO<List<TrendBaseBO>> findBaseTrend(LotteryTrendVO param) throws Exception {
		...
		for (TrendBaseBO trendBaseBO : target) {
			...
			bo.setRedList(genBaseDigits((SsqTrendBO)trendBaseBO,"r",Constants.NUM_33));
			bo.setBlueList(genBaseDigits((SsqTrendBO)trendBaseBO,"b",Constants.NUM_16));
			result.add(bo);
		}
	...
	public ResultBO<ColdHotOmitBo> findDrawColdHotOmit(LotteryVO param) throws Exception {
		...
		SsqTrendBO data =(SsqTrendBO) omit.getData();
		...
		//双色球选6个红球，1个蓝球
		List<NumTimeVo> omitData = dealWithOmitData(data,R,Constants.NUM_33);
		Collections.sort(omitData,compare);
		bo.setColdBaseList(new ArrayList<>(omitData.subList(Constants.NUM_0, Constants.NUM_6)));
		List<NumTimeVo> hotBaseList = omitData.subList(Constants.NUM_27, Constants.NUM_33);
		Collections.reverse(hotBaseList);
		bo.setHotBaseList(new ArrayList<>(hotBaseList));
		omitData =  dealWithOmitData(data,B,Constants.NUM_16);
		Collections.sort(omitData,compare);
		bo.setColdOtherList(new ArrayList<>(omitData.subList(Constants.NUM_0, Constants.NUM_1)));
		List<NumTimeVo> hotOtherList = omitData.subList(Constants.NUM_15, Constants.NUM_16);
	 ...
	 */
	private List<NumTimeVo> dealWithOmitData(SsqTrendBO baseTrend, String preField,int count) {
	...
	private List<Integer> genBaseDigits(SsqTrendBO baseTrend, String preField,int count) {
