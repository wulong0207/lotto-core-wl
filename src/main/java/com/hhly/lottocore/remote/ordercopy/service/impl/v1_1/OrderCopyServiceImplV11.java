package com.hhly.lottocore.remote.ordercopy.service.impl.v1_1;
...
import com.hhly.lottocore.persistence.ordercopy.dao.v1_1.OrderIssueInfoDaoMapperV11;
...
import com.hhly.lottocore.remote.ordercopy.service.v1_1.OrderIssueInfoServiceV11;
 ...
 * @company 益彩网络科技有限公司
 */
@Service("iOrderCopyServiceV11")
public class OrderCopyServiceImplV11 implements IOrderCopyServiceV11 {

    private static Logger logger = LoggerFactory.getLogger(OrderCopyServiceImplV11.class);

    @Autowired
    private MUserIssueInfoService mUserIssueInfoService;

    @Autowired
    private OrderIssueInfoServiceV11 orderIssueInfoServiceV11;
    ...
    @Autowired
    private OrderIssueInfoDaoMapperV11 orderIssueInfoDaoMapperV11;
    ...
    public ResultBO<?> insertOrderCopy(OrderIssueInfoVO vo) throws Exception {
        return orderIssueInfoServiceV11.addOrderIssueInfo(vo);
    }
...
//        mUserIssueLinkPO.setId(mUserIssueLinkVO.getId());
    ...
    public ResultBO<?> queryIssueInfo(QueryVO queryVO) {
        ...
        switch (queryVO.getQueryType()) {
            ...
            case 2:
            case 5:
                return orderIssueInfoServiceV11.queryIssueInfo(queryVO);
            case 4:
            case 3:
                return orderIssueInfoServiceV11.queryByQueryTypeThree(queryVO);
            default:
    ...
    public ResultBO<?> queryIssueInfoCount(QueryVO queryVO) {
        ...
        switch (queryVO.getQueryType()) {
            ...
            case 2:
            case 5:
                return ResultBO.ok(orderIssueInfoServiceV11.queryIssueInfoCount(queryVO));
            case 4:
            case 3:
                return ResultBO.ok(orderIssueInfoServiceV11.queryByQueryTypeThreeCount(queryVO));
            default:
                return ResultBO.err(MessageCodeConstants.PARAM_IS_FIELD);
        }
    }
    ...
    private ResultBO<?> processDif(QueryVO queryVO) {
        ...
        switch (queryVO.getQueryType()) {
            //首页实单查询，不一定是专家
            case 1:
                //根据tabType，设置issueType参数值
                if(queryVO.getTabType() == Constants.NUM_4){//推荐
                    queryVO.setIssueType(2);
                    queryVO.setLotteryCode(null);
                }else if(queryVO.getTabType() == Constants.NUM_1){//实单全部
                    queryVO.setIssueType(null);
                    queryVO.setLotteryCode(null);
                }else{//实单足球、篮球
                    if(queryVO.getTabType() == Constants.NUM_2){
                        queryVO.setLotteryCode(LotteryEnum.Lottery.FB.getName());
                    }else if(queryVO.getTabType() == Constants.NUM_3){
                        queryVO.setLotteryCode(LotteryEnum.Lottery.BB.getName());
                    }else{
                        return ResultBO.err(MessageCodeConstants.PARAM_IS_FIELD);
                    }
                    queryVO.setIssueType(1);
                }
...
//                Assert.paramNotNull(queryVO.getLotteryCode(), "lotteryCode");
                //设值 、排序
                queryVO.setFlag(1);
                queryVO.setSortField("a.is_top desc,a.is_recommend desc,a.max_roi desc,a.create_time");
...
//                Assert.paramNotNull(queryVO.getToken(), "token");
    ...
    public OrderFullDetailInfoBO buildOrderInfo(String orderCode) throws Exception {
        ...
        //中奖后，显示实际奖金
        if(orderBaseInfoBO.getWinningStatus() == OrderEnum.OrderWinningStatus.WINNING.getValue()){
            //税前奖金
        ...
        if (gameDetails.length > 1) {//有胆
            ...
            for (String gameCon : danDetails) {
                ...
                OrderMatchInfoBO orderMatchInfoBO = getOrderMatchInfo(lotteryCode,lotteryChildCode, getSystemCodeAndContent(gameCon, lotteryChildCode)[1],
                        systemCode,gameCon);
                orderMatchInfoBO.setIsDan(Constants.NUM_1);
    ...
    private List<BetContentBO> getBetContents(Integer lotteryCode,Integer lotteryChildCode,String content,JczqOrderBO jczqOrderBO,JclqOrderBO jclqOrderBO){
        ...
        for(String sps : betSps.split(SymbolConstants.COMMA)){
            ...
            if(lotteryCode == JCZQConstants.ID_JCZQ_B){//足球
                ...
                betContentBO.setPanKou(getLQInfoFromSingleBetContent(content,lotteryChildCode));//让分：如[+1]
            }
    ...
    public ResultBO<?> findOrderCopyIssueInfoBOById(Long id, String token) throws Exception {
        OrderCopyInfoBO bo = orderIssueInfoServiceV11.findOrderCopyIssueInfoBOById(id);
    ...
    public ResultBO<?> validateOrderCopy(String orderCode, String token) throws Exception {
        ...
        result = orderIssueInfoServiceV11.validateOrderCopy(orderCode, userInfoBO,null);
        if (!result.isError()) {
            result.setData(null);
        }
        return result;
    ...
    public ResultBO<?> getHandicapChange(Long issueOrderId, String token)throws Exception{
        ...
        OrderCopyInfoBO orderCopyInfoBO = orderIssueInfoServiceV11.findOrderCopyIssueInfoBOById(issueOrderId);
        ...
        //1、公开可见 2、关注后可见（用户已关注），则可见
        if(orderCopyInfoBO.getOrderVisibleType() == 2 || (orderCopyInfoBO.getOrderVisibleType() == 4 && focus)){
                    ...
                    if(!ObjectUtil.isBlank(newHandicap)){
                        for(HashMap<String, HandicapBO> newMatchMap : newHandicap){
                            if(!ObjectUtil.isBlank(newMatchMap.get(systemCode)) && !ObjectUtil.isBlank(newMatchMap.get(systemCode).getNewSp())
                                    && newMatchMap.get(systemCode).getPlay().equals(initMatchMap.get(systemCode).getPlay())){//盘口有变化
                                String newSp = newMatchMap.get(systemCode).getNewSp();
                                String initSp = initMatchMap.get(systemCode).getNewSp();
                                if(newSp.contains(SymbolConstants.ADD) || newSp.contains(SymbolConstants.TRAVERSE_SLASH)){
                                ...
                                if(!newSp.equals(initSp)){
                                    HandicapBO handicapBO = new HandicapBO();
                                    handicapBO.setSerNum(newMatchMap.get(systemCode).getSerNum());//赛事编号
                                    handicapBO.setPreSp(initSpWithoutSymbol);//方案赔率
                                    handicapBO.setNewSp(newSp);//最新盘口SP
    ...
    private void setInitHandicapMap(List<HashMap<String, HandicapBO>> initHandicap, List<OrderDetailInfoBO> listOrderDetailInfoBO) {
        for(OrderDetailInfoBO orderDetailInfoBO : listOrderDetailInfoBO){
            ...
            String[] betContents = FormatConversionJCUtil.singleBetContentAnalysis(orderDetailInfoBO.getBetContent());
    ...
    private void jclqDxf(String systemCode, JclqOrderBO jclqOrderBO, HashMap<String, HandicapBO> newMatchBet, boolean flag) {
        ...
        String releaseTime;
        String dxf;
        resultBO = jcDataService.findJclqSssHistorySpData(jclqOrderBO.getSportMatchInfoId());
        ...
        if(!ObjectUtil.isBlank(listSp)){
            ...
            dxf = listSp.get(0)[0];//让分值最新盘口
        }else{//为空时，取当前时间，sp盘口取对阵sp值
            releaseTime = DateUtil.getNow();
    ...
    private void jclqRF(String systemCode, JclqOrderBO jclqOrderBO, HashMap<String, HandicapBO> newMatchBet, boolean flag) {
        ...
        resultBO = jcDataService.findJclqWfHistorySpData(jclqOrderBO.getSportMatchInfoId(), SportEnum.WfTypeEnum.LET.getValue());
        listSp = resultBO.getData();
        if(!ObjectUtil.isBlank(listSp)){
            releaseTime = listSp.get(0)[3];//更新时间
            lefWf = listSp.get(0)[0];//让分值最新盘口
        }else{//为空时，取当前时间，sp盘口取对阵sp值
            releaseTime = DateUtil.getNow();//当前时间
...
//            return result;
//        }
        OrderCopyPayInfoBO orderCopyPayInfoBO = orderIssueInfoDaoMapperV11.getCopyOrderInfoForPay(orderIssueId);
