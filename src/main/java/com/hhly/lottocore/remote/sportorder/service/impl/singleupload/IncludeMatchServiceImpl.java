 ...
 * @desc 竞彩足球 包含场次单式上传
 * @author YiJian
 * @date 2017年6月9日 下午12:30:31
 * @company 深圳益彩网络科技有限公司
 * @version v1.0
 */
@Service("includeMatchService")
public class IncludeMatchServiceImpl extends SingleUploadOrderValidateMethod implements IncludeMatchService {
...
//		String[] uploadDatas = FileUtil.readFileLine(singleUploadVO.getFilePath(), EncodingType.UFT8.getShortName());
		...
		singleUploadVO.setPlayCode(JCZQConstants.getShortName(singleUploadVO.getLotteryChildCode()));
		...
		if (!ObjectUtil.isBlank(singleUploadVO.getShiftContent())) {
			ResultBO<?> result = getStandarTransfer(singleUploadVO.getShiftContent(),singleUploadVO.getPlayCode());
			if(result.isError()){
				return result;
			}
